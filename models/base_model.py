...
class BaseModel(ABC):
    ...
    def __init__(self, gpu_ids, isTrain, checkpoints_dir, name, preprocess, lr_policy, lr_decay_iters):
    ...
    def setup(self, continue_train=False, epoch='latest', load_iter=0, verbose=True,
              n_epochs=100, epoch_count=1, n_epochs_decay=100):
        """Load and print networks; create schedulers
        :param continue_train: continue training: load the latest model
        :param epoch: which epoch to load? set to latest to use latest cached model
        :param load_iter: which iteration to load? if load_iter > 0, the code will load models by iter_[load_iter]; otherwise, the code will load models by [epoch]
        :param verbose: if specified, print more debugging information
        ...
        """
        if self.isTrain:
            self.schedulers = [networks.get_scheduler(optimizer=optimizer,
